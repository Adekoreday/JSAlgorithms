...
function signAndSend(newtx) {
  ...
  newtx.signatures = newtx.tosign.map(function (tosign) {
    newtx.pubkeys.push(source.public);  //add public key
    return key.sign(new buffer.Buffer(tosign, "hex")).toDER().toString("hex"); //sign the private key
  });
